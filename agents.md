...
## Architecture Notes
- Modules: `aipod.py` (Typer entry + wiring), `aipod/podman.py` (subprocess wrapper), `aipod/system.py` (env checks, GPU detection), `aipod/pods.py` (service specs), `aipod/logging.py` (Rich console).
- Pod specs include names, images, ports, env, volumes, and GPU requirements. Easy to extend mapping for new services.
